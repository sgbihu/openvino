...
#    endif
    ...
    class CustomThreadLocal : public ThreadLocal<std::shared_ptr<Stream>>, public std::enable_shared_from_this<CustomThreadLocal> {
    public:
        ...
        struct ThreadCleaner {
            ThreadCleaner (std::weak_ptr<CustomThreadLocal> parent, std::thread::id thread_id) : _parent(parent), _thread_id(thread_id) {}
            ~ThreadCleaner() {
        ...
        private:
            ...
            std::weak_ptr<CustomThreadLocal> _parent;
        };
        std::shared_ptr<Stream> local() {
            ...
            std::weak_ptr<CustomThreadLocal> parent = shared_from_this();
            thread_local ThreadCleaner t_cleaner(parent, id);
